...
namespace IMTest.Tests
{
    public abstract class IMTestTestBase : AbpIntegratedTestBase
    {
        ...
        protected void UsingDbContext(Action<IMTestDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<IMTestDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<IMTestDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<IMTestDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<IMTestDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<IMTestDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<IMTestDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<IMTestDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<IMTestDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<IMTestDbContext, Task<T>> func)
        {
            T result;
